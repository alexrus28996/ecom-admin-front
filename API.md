...
| `PATCH /api/admin/orders/:id` | Admin (role) | Working (manual review) | Body `{ "status?", "paymentStatus?" }` | Enfor ces linear status transitions (pending → paid → shipped → delivered or cancelled) and releases reservations on cancellation. |
